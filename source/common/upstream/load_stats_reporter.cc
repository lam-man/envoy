      ...
      for (const HostVector& hosts : host_set->hostsPerLocality().get()) {
        ...
        if (rq_success + rq_error + rq_active != 0) {
          ...
          if (message_->report_endpoint_granularity()) {
            for (const HostSharedPtr& host : hosts) {
              ENVOY_LOG(info, "Load report endpoint {}:{} count {}", host->address()->asString(), host->address()->ip()->port(), hosts.size());
              ...
              // add endpoint stats (successfull requests, requests in progress, error requests, and issued requests) 
              upstream_endpoint_stats->set_total_successful_requests(host->stats().rq_success_.latch());
              upstream_endpoint_stats->set_total_requests_in_progress(host->stats().rq_active_.value());
